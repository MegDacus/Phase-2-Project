...
function AnimalPage({animalArray, animalName}) {
    ...
    return(
        <Container className='text-center'>
            ...
            <h4 className="heading">Available {animalName}</h4>
            ...
            <CardGroup className="row row-cols-1 row-cols-md-3 g-4">
                {animalArray.map((animal, index) => <AnimalCard key={index} image={animal.image} name={animal.name} setShow={setShow}/>
                )}
            </CardGroup>
        </Container>
    )
